...
/*
    Why dynamic allocation is preferred over fixed size arrays:
     A static array such as char lines[1000][1000] reserves 1MB of memory even if the user enters only one line.
     With dynamic allocation, the program uses only the memory actually needed (about 100 bytes for 10 short lines).
     It can easily scale to handle thousands or even hundreds of thousands of lines.
     Using shrinkToFit helps free unused memory after deletions, reducing overall waste.
*/
